...
describe('frogbot bin', () => {
  ...
  it.todo('dispatches `generate:importmap` to the generateImportMap command');
  it.todo('treats the command as case-insensitive');
  it.todo(
    'prints `[frogbot] usage: frogbot <start|dev|generate:types|generate:importmap>` and exits with code 2 on no args',
  );
