...
description: Context allows you to pass in extra data that can be shared between hooks
keywords: hooks, context, frogbot context, frogbotcontext, data, extra data, shared data, shared, extra
---
...
2. **Preventing infinite loops**: Calling `frogbot.update()` on the same document that triggered an `afterChange` hook will create an infinite loop, control the flow by assigning a no-op condition to context
3. **Passing data to Local API**: Setting values on the `req.context` and pass it to `frogbot.create()` you can provide additional data to hooks without adding extraneous fields.
