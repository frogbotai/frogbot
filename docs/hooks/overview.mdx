...
<Warning>
  ...
  and should not be confused. [More details](/admin/react-hooks).
</Warning>
...
| **`req`**           | The `PayloadRequest` object that extends [Web Request](https://developer.mozilla.org/en-US/docs/Web/API/Request). Contains currently authenticated `user` and the Local API instance `frogbot`. |
