...
description: Hooks can be added to any Global and allow you to validate data, flatten locales, hide protected fields, remove fields and more.
...
Global Hooks are [Hooks](/hooks/overview) that run on [Global](/configuration/globals) Documents. They allow you to execute your own logic during specific events of the Document lifecycle.
...
All Global Hooks accept an array of [synchronous or asynchronous functions](/hooks/overview#async-vs-synchronous). Each Global Hook receives specific arguments based on its own type, and has the ability to modify specific outputs.
...
```ts
import type { GlobalBeforeOperationHook } from 'frogbot'
...
```ts
import type { GlobalBeforeValidateHook } from 'frogbot'
import type { SiteSettings } from '@/frogbot-types'
...
```ts
import type { GlobalBeforeChangeHook } from 'frogbot'
import type { SiteSettings } from '@/frogbot-types'
...
```ts
import type { GlobalAfterChangeHook } from 'frogbot'
import type { SiteSettings } from '@/frogbot-types'
...
```ts
import type { GlobalBeforeReadHook } from 'frogbot'
...
```ts
import type { GlobalAfterReadHook } from 'frogbot'
import type { SiteSettings } from '@/frogbot-types'
...
FrogBot exports a type for each Global hook which can be accessed as follows:
