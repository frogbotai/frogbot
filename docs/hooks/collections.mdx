...
description: You can add hooks to any Collection, several hook types are available including beforeChange, afterRead, afterDelete and more.
...
Collection Hooks are [Hooks](/hooks/overview) that run on Documents within a specific [Collection](/configuration/collections). They allow you to execute your own logic during specific events of the Document lifecycle.
...
<Note>
  You can also set hooks on the field-level to isolate hook logic to specific
  fields. [More details](/hooks/fields).
</Note>

## Config Options

All Collection Hooks accept an array of [synchronous or asynchronous functions](/hooks/overview#async-vs-synchronous). Each Collection Hook receives specific arguments based on its own type, and has the ability to modify specific outputs.
...
```ts
import type { CollectionBeforeOperationHook } from 'frogbot'
...
```ts
import type { CollectionBeforeValidateHook } from 'frogbot'
...
After a document is created or updated, the `afterChange` hook runs. This hook is helpful for recalculating statistics, syncing user profile changes to a CRM, and more.
...
```ts
import type { CollectionBeforeReadHook } from 'frogbot'
import type { Post } from '@/frogbot-types'
...
```ts
import type { CollectionAfterReadHook } from 'frogbot'
import type { Post } from '@/frogbot-types'
...
```ts
import type { CollectionBeforeDeleteHook } from 'frogbot';
...
```ts
import type { CollectionAfterDeleteHook } from 'frogbot';
...
```ts
import type { CollectionAfterOperationHook } from 'frogbot'
...
```ts
import type { CollectionAfterErrorHook } from 'frogbot';
...
```ts
import type { CollectionBeforeLoginHook } from 'frogbot'
...
```ts
import type { CollectionAfterLoginHook } from 'frogbot';
...
```ts
import type { CollectionAfterLogoutHook } from 'frogbot';
...
```ts
import type { CollectionAfterMeHook } from 'frogbot';
...
```ts
import type { CollectionAfterRefreshHook } from 'frogbot';
...
```ts
import type { CollectionAfterForgotPasswordHook } from 'frogbot'
...
```ts
import type { CollectionRefreshHook } from 'frogbot'
...
```ts
import type { CollectionMeHook } from 'frogbot'
...
FrogBot exports a type for each Collection hook which can be accessed as follows:
