...
title: Field Hooks
sidebarTitle: Fields
description: Hooks can be added to any fields, and optionally modify the return value of the field before the operation continues.
...
Field Hooks are [Hooks](/hooks/overview) that run on Documents on a per-field basis. They allow you to execute your own logic during specific events of the Document lifecycle. Field Hooks offer incredible potential for isolating your logic from the rest of your [Collection Hooks](/hooks/collections).
...
<Warning>
  Due to GraphQL's typed nature, changing the type of data that you return from
  a field will produce errors in the [GraphQL API](/graphql/overview). If you
  need to change the shape or type of data, consider [Collection
  Hooks](/hooks/collections) instead.
</Warning>
...
| Option                      | Description                                                                                                                                                                                    |
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`collection`**            | The [Collection](/configuration/collections) in which this Hook is running.                                                                                                                  |
...
| **`siblingData`**           | The data of sibling [fields](/fields/overview) adjacent to the field that the Hook is running against.                                                                                          |
...
<Tip>
  ...
  executing. For example, if you are writing a `beforeChange` hook, you may want
  to perform different logic based on if the current `operation` is `create` or
  `update`.
</Tip>
...
By Default, unique and required text fields FrogBot will append "- Copy" to the original document value. The default is not added if your field has its own, you must return non-unique values from your beforeDuplicate hook to avoid errors or enable the `disableDuplicate` option on the collection.
...
FrogBot exports a type for field hooks which can be accessed and used as follows:

```ts
import type { FieldHook } from 'frogbot'
...
Here's a real-world example using generated FrogBot types:
...
<Tip>
  When defining field hooks, use the three generic parameters for full type
  safety: document type, field value type, and sibling data type. This provides
  autocomplete and type checking for all hook arguments.
</Tip>
