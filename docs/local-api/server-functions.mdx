...
- You can still use utilities such as `getFrogbot()`.
...
```ts
import { login } from '@frogbotai/next/auth'
```
...
```ts
import { logout } from '@frogbotai/next/auth'
```
...
```ts
import { refresh } from '@frogbotai/next/auth'
```
