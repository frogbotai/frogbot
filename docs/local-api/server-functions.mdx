...
description: Learn to use Local API operations with Server Functions in FrogBot to manage server-side logic, data interactions, and custom workflows directly within your CMS.
keywords: server functions, local API, FrogBot, server-side logic, custom workflows, data management, TypeScript, Node.js, backend
---
...
All Local API operations can be used within server functions, allowing you to interact with FrogBot's backend securely.
...
- Once the function structure is in place, call the Local API operation `frogbot.create()` and pass in the relevant data.
...
To update a document instead of creating one, you would use `frogbot.update()` with the relevant data and **passing the document ID.**
...
export async function updatePost(id, data) {
  ...
  try {
    const post = await frogbot.update({
      collection: 'posts',
...
In this example, we will check if a user is authenticated using FrogBot's authentication system. Here's how it works:

- First, we use the headers function from `next/headers` to retrieve the request headers.
- Next, we pass these headers to `frogbot.auth()` to fetch the user's authentication details.
...
- Use `frogbot.create()` to create a new post document with both the document data and file
...
## Reusable FrogBot Server Functions
...
```ts
import { login } from '@frogbot/next/auth'
```

The login function needs your FrogBot config, which cannot be imported in a client component. To work around this, create a simple server function like the one below, and call it from your client.
...
```ts
import { refresh } from '@frogbot/next/auth'
```

As with login and logout, you need to pass your FrogBot config to this function. Create a helper server function like the one below. Passing the config directly to the client is not possible and will throw errors.
...
export async function createPost(data) {
  try {
    const frogbot = await getPayload({ config })
    return await frogbot.create({ collection: 'posts', data })
  } catch (error) {
...
```ts
import { UnauthorizedError } from 'frogbot'
...
export async function deletePost(postId, user) {
  ...
  const frogbot = await getPayload({ config })
  return await frogbot.delete({ collection: 'posts', id: postId })
}
```
