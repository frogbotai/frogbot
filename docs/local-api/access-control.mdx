...
description: Learn how to implement and enforce access control in FrogBot's Local API operations, ensuring that the right permissions are respected during data manipulation.
keywords: server functions, local API, FrogBot, access control, permissions, user context, server-side logic, custom workflows, data management, headless CMS, TypeScript, Node.js, backend
---

In FrogBot, local API operations **override access control by default**. This means that operations will run without checking if the current user has permission to perform the action. This is useful in certain scenarios where access control is not necessary, but it is important to be aware of when to enforce it for security reasons.
...
const test = await frogbot.create({
  collection: 'users',
  data: {
    email: 'test@test.com',
    password: 'test',
...
```ts
const authedCreate = await frogbot.create({
  collection: 'users',
