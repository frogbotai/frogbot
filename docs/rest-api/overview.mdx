...
description: FrogBot generates a fully functional REST API from your Collection and Global configs.
...
| Operation     | Method | Path                          | Description            |
| ------------- | ------ | ----------------------------- | ---------------------- |
| Get Global    | `GET`  | `/api/globals/{global-slug}`  | Get a global by slug   |
| Update Global | `POST` | `/api/globals/{global-slug}`  | Update a global by slug |

## Preferences

In addition to the dynamically generated endpoints above FrogBot also has REST endpoints to manage the admin user [preferences](/admin/preferences) for data specific to the authenticated user.
...
| **`handler`** | A function that accepts **req** - `PayloadRequest` object which contains [Web Request](https://developer.mozilla.org/en-US/docs/Web/API/Request) properties, currently authenticated `user` and the Local API instance `frogbot`.                          |
| **`root`**    | When `true`, defines the endpoint on the root Next.js app, bypassing FrogBot handlers and the `routes.api` subpath. Note: this only applies to top-level endpoints of your FrogBot Config, endpoints defined on `collections` or `globals` cannot be root. |
...
The best, fully type-safe way to query FrogBot REST API is to use the SDK package, which can be installed with:

```bash
pnpm add @frogbot/sdk
```
...
```ts
import { PayloadSDK } from '@frogbot/sdk'
...
For projects without a `frogbot-types.ts` file, or when working with multiple FrogBot configs, you can manually pass the types as a generic:

```ts
import { PayloadSDK } from '@frogbot/sdk'
import type { Config } from './frogbot-types'
...
const result = await sdk.login({
  collection: 'users',
  data: { email: 'dev@frogbot.com', password: '123456' },
})
