...
title: Drafts
sidebarTitle: Drafts
description: Enable drafts on collection documents and build true preview environments for your data.
...
FrogBot's Draft functionality builds on top of Versions to let you change collection documents but publish only when you're ready. This functionality allows you to build powerful Preview environments for your data, where you can make sure your changes look good before publishing documents.

<Warning>
  Drafts rely on Versions being enabled in order to function.
</Warning>

By enabling Versions with Drafts, your collections can maintain newer, unpublished versions of documents. It's useful when you want to update a document and save your progress without publishing it immediately. Drafts are especially helpful when building preview implementations.

## Options

Collections support two ways to configure drafts. Set `versions.drafts` to `true`, or pass an object to configure draft properties.
...
By enabling drafts on a collection, FrogBot will **automatically inject a new field into your schema** called `_status`. The `_status` field is used internally by FrogBot to store whether a document is set to `draft` or `published`.
...
<Tip>
  If drafts are enabled on your collection, important and powerful
...
When `draft` is set to `true` while reading a document, **FrogBot will return the most recent version from the versions table**, regardless of whether it's a draft or published document.
...
However, if you pass `draft: true` to the read operation, FrogBot will return the most recent version from the versions table. In the scenario above with two draft versions, you'll get the latest (second) draft.
...
<Warning>
  ...
  FrogBot makes this extremely simple and puts the power completely in your
  hands.
</Warning>
...
You can use `update` [Access Control](/access-control/collections#update) to restrict who can publish documents by returning a [query constraint](/queries/overview) on the `_status` field. When this constraint prevents publishing, the Admin UI automatically hides the Publish and Unpublish buttons.
...
The `update` function returns a query constraint instead of a boolean. FrogBot appends this constraint to the update query, so only documents where `_status` is not `published` can be updated by editors. This also blocks scheduled publish jobs from executing for those users.

The `create` function uses a `data` check instead since there is no existing document to query against.

## Scheduled publish

FrogBot provides for an ability to schedule publishing / unpublishing events in the future, which can be helpful if you need to set certain documents to "go live" at a given date in the future, or, vice versa, revert to a draft state after a certain time has passed.

Enable this functionality on a collection with the `versions.drafts.schedulePublish: true` property.

<Warning>
  **Important:** if you are going to enable scheduled publish / unpublish, you
  need to make sure your FrogBot app is set up to process
...
If a document is published, the FrogBot Admin UI will be updated to show an "unpublish" button at the top of the sidebar, which will "unpublish" the currently published document. Consider this as a way to "revert" a document back to a draft state. On the API side, this is done by simply setting `_status: 'draft'` on any document.

## Reverting to published

If a document is published, and you have made further changes which are saved as a draft, FrogBot will show a "revert to published" button at the top of the sidebar which will allow you to reject your draft changes and "revert" back to the published state of the document. Your drafts will still be saved, but a new version will be created that will reflect the last published state of the document.
