...
description: Keep a version history or audit log of changes and publish collection documents and globals.
...
// Result will be a paginated set of Versions.
// See /queries/pagination for more.
const result = await frogbot.findGlobalVersions({
...
```js
// Result will be a Post document.
const result = await frogbot.findGlobalVersionByID({
  slug: 'header', // required
  id: '507f1f77bcf86cd799439013', // required
  depth: 2,
  locale: 'en',
...
```js
// Result will be the restored global document.
const result = await frogbot.restoreGlobalVersion({
  slug: 'header', // required
  id: '507f1f77bcf86cd799439013', // required
  depth: 2,
  user: dummyUser,
