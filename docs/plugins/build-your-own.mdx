...
description: Starting to build your own plugin? Find everything you need and learn best practices with the FrogBot plugin template.
...
Building your own [FrogBot Plugin](/plugins/overview) is easy, and if you're already familiar with FrogBot then you'll have everything you need to get started. You can either start from scratch or use the [Plugin Template](#plugin-template) to get up and running quickly.

<Tip>
  To use the template, run `npx create-frogbot-app@latest --template plugin`
  directly in your terminal.
</Tip>

Our plugin template includes everything you need to build a full life-cycle plugin:

- Example files and functions for extending the FrogBot Config
...
In the [FrogBot Plugin Template](https://github.com/frogbot/frogbot/tree/main/templates/plugin), you will see a common file structure that is used across plugins:
...
In the root folder, you will see various files related to the configuration of the plugin. We set up our environment in a similar manner in FrogBot core and across other projects. The only two files you need to modify are:
...
The purpose of the **dev** folder is to provide a sanitized local FrogBot project so you can run and test your plugin while you are actively developing it.
...
If you're using the plugin template, the dev folder is built out for you and the `samplePlugin` has already been installed in `dev/frogbot.config.ts`.
...
You can add to the `dev/frogbot.config.ts` and build out the dev project as needed to test your plugin.
...
A good test suite is essential to ensure quality and stability in your plugin. FrogBot typically uses [Jest](https://jestjs.io/); a popular testing framework, widely used for testing JavaScript and particularly for applications built with React.
...
describe('Plugin tests', () => {
  ...
  it('seeds data accordingly', async () => {
    const newCollectionQuery = await frogbot.find({
...
```ts
export const seed = async (frogbot: FrogBot): Promise<void> => {
  frogbot.logger.info('Seeding data...')
