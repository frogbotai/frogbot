...
description: Troubleshooting Common Issues in FrogBot
...
All `frogbot` and `@frogbot/*` packages must be on exactly the same version and installed only once.
...
This happens because one package imports a hook (most commonly `useConfig`) from _version A_ while the context provider comes from _version B_. The fix is always the same: make sure every FrogBot-related and React package resolves to the same module.
...
`@frogbot/ui` intentionally contains two bundles of itself, so you may see dual paths even when everything is correct. Inside the FrogBot Admin UI you must import only:

- `@frogbot/ui`
- `@frogbot/ui/rsc`
- `@frogbot/ui/shared`

Any other deep import such as `@frogbot/ui/elements/Button` should **only** be used in your own frontend, outside of the FrogBot Admin Panel. Those deep entries are published un-bundled to help you tree-shake and ship a smaller client bundle if you only need a few components from `@frogbot/ui`.

### Fixing dependency issues

These steps assume `pnpm`, which the FrogBot team recommends and uses internally. The principles apply to other package managers like npm and yarn as well. Do note that yarn 1.x is not supported by FrogBot.
...
- Run [Syncpack](https://www.npmjs.com/package/syncpack) to enforce identical versions of every `@frogbot/*`, `react`, and `react-dom` reference.
...
This is a common pitfall when using a monorepo setup with multiple packages. In this case, ensure that all packages in the monorepo use the same version of `frogbot`, `@frogbot/*`, `next`, `react`, and `react-dom`. You can use pnpm with workspaces to manage dependencies across packages in a monorepo effectively. Unfortunately this error becomes harder to debug inside a monorepo due to how package managers hoist dependencies as well as resolve them.
...
Where possible it's best to install the FrogBot dependencies at the root of the monorepo to ensure only one version and one instance is installed across all packages.

## "Unauthorized, you must be logged in to make this request" when attempting to log in

This means that your auth cookie is not being set or accepted correctly upon logging in. To resolve check the following settings in your FrogBot Config:
...
This error likely means that the auth cookie that FrogBot sets after logging in successfully is being rejected because of misconfiguration.
...
If you are using the `--experimental-https` flag when starting your FrogBot server, you may run into issues with your WebSocket connection for HMR (Hot Module Reloading) in development mode.
...
You may see some generic errors such as the following when FrogBot is unable to connect to your database:
...
If the issue persists only in FrogBot, check your database password for special characters. Some special characters need to be URI encoded in order to be parsed correctly from the connection string. You can pass this string through `encodeURIComponent()` to get the encoded version.
