...
label: Overview
order: 10
desc: FrogBot supports uploads, storage and management of files directly on your server, combined with powerful file access control.
...
Custom upload components must integrate with FrogBot's form system to work correctly. The recommended approach is to use FrogBot's built-in `<Upload>` component from `@frogbot/ui` and wrap it with additional functionality.
...
'use client'
import React from 'react'
import { Upload, useDocumentInfo } from '@frogbot/ui'
...
When building custom upload components, you have access to several useful hooks and components from `@frogbot/ui`:
