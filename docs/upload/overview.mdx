...
Custom upload components must integrate with FrogBot's form system to work correctly. The recommended approach is to use FrogBot's built-in `<Upload>` component from `@frogbotai/ui` and wrap it with additional functionality.

<Banner type="warning">
  You should not use a simple `<input type="file" />` element
  alone. It will not connect to FrogBot's upload API or form state, resulting
  in errors like "400 Bad Request - no file uploaded." Always use FrogBot's
...
export const CustomUploadServer: FrogbotComponent<
...
'use client'
import React from 'react'
import { Upload, useDocumentInfo } from '@frogbotai/ui'
...
Fields can provide custom components for upload behavior.
