...
description: FrogBot provides additional storage adapters to handle file uploads. These adapters allow you to store files in different locations, such as Amazon S3, Vercel Blob Storage, Google Cloud Storage, Uploadthing, and more.
keywords: uploads, images, media, storage, adapters, s3, vercel, google cloud, azure
---

FrogBot offers additional storage adapters to handle file uploads. These adapters allow you to store files in different locations, such as Amazon S3, Vercel Blob Storage, Google Cloud Storage, and more.

| Service              | Package                                                                                                           |
| -------------------- | ----------------------------------------------------------------------------------------------------------------- |
| Vercel Blob          | [`@frogbotai/storage-vercel-blob`](https://github.com/frogbot/frogbot/tree/main/packages/storage-vercel-blob) |
| AWS S3               | [`@frogbotai/storage-s3`](https://github.com/frogbot/frogbot/tree/main/packages/storage-s3)                   |
| Azure                | [`@frogbotai/storage-azure`](https://github.com/frogbot/frogbot/tree/main/packages/storage-azure)             |
| Google Cloud Storage | [`@frogbotai/storage-gcs`](https://github.com/frogbot/frogbot/tree/main/packages/storage-gcs)                 |
| Uploadthing          | [`@frogbotai/storage-uploadthing`](https://github.com/frogbot/frogbot/tree/main/packages/storage-uploadthing) |
| R2                   | [`@frogbotai/storage-r2`](https://github.com/frogbot/frogbot/tree/main/packages/storage-r2)                   |

## Configuration

Storage adapters are registered under the top-level `storage` key in your FrogBot config. This placement ensures that upload hooks, static file handlers, and presigned-URL endpoints are wired up before any plugin modifies the config.
...
```sh
npx @frogbotai/codemod --transform migrate-storage-adapters-to-config
```
...
[`@frogbotai/storage-vercel-blob`](https://www.npmjs.com/package/@frogbotai/storage-vercel-blob)
...
[`@frogbotai/storage-s3`](https://www.npmjs.com/package/@frogbotai/storage-s3)
...
Cloudflare R2 exposes an S3-compatible API, so you can use `@frogbotai/storage-s3` to connect to it. This is the recommended approach when deploying to Vercel, Netlify, or any Node.js environment. (The `@frogbotai/storage-r2` adapter is for Cloudflare Workers only, where R2 is available as a native bucket binding.)
...
- `disablePayloadAccessControl: true` — bypasses FrogBot's file proxy so URLs point directly to your public R2 domain

## Azure Blob Storage

[`@frogbotai/storage-azure`](https://www.npmjs.com/package/@frogbotai/storage-azure)
...
[`@frogbotai/storage-gcs`](https://www.npmjs.com/package/@frogbotai/storage-gcs)
...
[`@frogbotai/storage-uploadthing`](https://www.npmjs.com/package/@frogbotai/storage-uploadthing)
...
[`@frogbotai/storage-r2`](https://www.npmjs.com/package/@frogbotai/storage-r2)
...
- Pass in the R2 bucket binding to the `bucket` option, this should be done in the environment where FrogBot is running (e.g. Cloudflare Worker).
...
If you need to create a custom storage adapter, you can use the [`@frogbotai/plugin-cloud-storage`](https://www.npmjs.com/package/@frogbotai/plugin-cloud-storage) package. This package is used internally by the storage adapters mentioned above.

### Installation#custom-installation

`pnpm add @frogbotai/plugin-cloud-storage`
...
```ts
import { buildConfig } from 'frogbot'
import { cloudStoragePlugin } from '@frogbotai/plugin-cloud-storage'
...
This plugin is configurable to work across many different FrogBot collections. A `*` denotes that the property is required.

| Option               | Type                                | Description                                                                                                                                                                                                   |
| -------------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `alwaysInsertFields` | `boolean`                           | When enabled, fields (like the prefix field) will always be inserted into the collection schema regardless of whether the plugin is enabled. This will be enabled by default in FrogBot v4. Default: `false`. |
...
| Option                        | Type                                                                                                              | Description                                                                                                                                                                                                   |
| ----------------------------- | ----------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `adapter` \*                  | [Adapter](https://github.com/frogbot/frogbot/blob/main/packages/plugin-cloud-storage/src/types.ts#L49)         | Pass in the adapter that you'd like to use for this collection. You can also set this field to `null` for local development if you'd like to bypass cloud storage in certain scenarios and use local storage. |
| `disableLocalStorage`         | `boolean`                                                                                                         | Choose to disable local storage on this collection. Defaults to `true`.                                                                                                                                       |
| `disablePayloadAccessControl` | `true`                                                                                                            | Set to `true` to disable FrogBot's Access Control. [More](#frogbot-access-control)                                                                                                                            |
| `generateFileURL`             | [GenerateFileURL](https://github.com/frogbot/frogbot/blob/main/packages/plugin-cloud-storage/src/types.ts#L67) | Override the generated file URL with one that you create.                                                                                                                                                     |
...
FrogBot ships with [Access Control](../access-control/overview) that runs _even on statically served files_. The same `read` Access Control property on your `upload`-enabled collections is used, and it allows you to restrict who can request your uploaded files.

To preserve this feature, by default, this plugin _keeps all file URLs exactly the same_. Your file URLs won't be updated to point directly to your cloud storage source, as in that case, FrogBot's Access control will be completely bypassed and you would need public readability on your cloud-hosted files.
