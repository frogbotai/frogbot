...
Authentication is a critical part of any application. FrogBot provides a secure, portable way to manage user accounts out of the box. FrogBot Authentication is designed to be used in both the [Admin Panel](/admin/overview), as well as your own external applications, completely eliminating the need for paid, third-party platforms and services.
...
To enable auto-login, set the `autoLogin` property in the [FrogBot Config](/admin/overview#admin-options):
...
To enable auto-refresh for user tokens, set `autoRefresh: true` in the [FrogBot Config](/admin/overview#admin-options) to:
