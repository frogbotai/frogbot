...
FrogBot does not require you to define a users collection. If no auth collection exists, Payload supplies its default `users` collection. If exactly one auth collection exists, FrogBot uses it as the owner relationship for chat threads regardless of its slug. With multiple auth collections, set `admin.user` to choose one.
