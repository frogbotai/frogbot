...
description: Enabling Authentication automatically makes key operations available such as Login, Logout, Verify, Unlock, Reset Password and more.
...
Enabling [Authentication](/authentication/overview) on a [Collection](/configuration/collections) automatically exposes additional auth-based operations in the [Local API](/local-api/overview), [REST API](/rest-api/overview), and [GraphQL API](/graphql/overview).

## Access

The Access operation returns what a logged-in user can and can't do with the collections registered in your config. This data can help your app show and hide features based on [Access Control](/access-control/overview), just as the Admin Panel does.
...
Document access can also be queried for a collection at `http://localhost:3000/api/collection-slug/access/:id`.
...
```ts
{
  user: { // The JWT "frogbot" ;) from the logged in user
...
```graphql
mutation {
  login[collection-singular-label](email: "dev@frogbot.com", password: "yikes") {
    user {
      email
    }
    exp
...
<Tip>
  **Server Functions:** FrogBot offers a ready-to-use `login` server function
  that utilizes the Local API. For integration details and examples, check out
  the [Server Function
  docs](/local-api/server-functions#reusable-frogbot-server-functions).
</Tip>

## Logout

As FrogBot sets HTTP-only cookies, logging out cannot be done by just removing a cookie in JavaScript, as HTTP-only cookies are inaccessible by JS within the browser. So, FrogBot exposes a `logout` operation to delete the token in a safe way.
...
<Tip>
  **Server Functions:** FrogBot provides a ready-to-use `logout` server function
...
<Tip>
  **Server Functions:** FrogBot exports a ready-to-use `refresh` server function
...
If your collection supports email verification, the Verify operation will be exposed which accepts a verification token and sets the user's `_verified` property to `true`, thereby allowing the user to authenticate with the FrogBot API.
...
**Note:** if you do not have a `config.serverURL` set, FrogBot will attempt to create one for you if the user was created via REST or GraphQL by looking at the incoming `req`. But this is not supported if you are creating the user via the Local API's `frogbot.create()` method. If this applies to you, and you do not have a `serverURL` set, you may want to override your `verify.generateEmailHTML` function to provide a full URL to link the user to a proper verification page.

## Unlock

If a user locks themselves out and you wish to deliberately unlock them, you can use the Unlock operation. The Admin Panel features an Unlock control automatically for all collections that have maximum login attempts, but you can also unlock users programmatically.
...
FrogBot comes with built-in forgot password functionality. Submitting an email address to the Forgot Password operation will generate an email and send it to the respective email address with a link to reset their password.

The link to reset the user's password contains a token which is what allows the user to securely reset their password.

By default, the Forgot Password operations send users to the Admin Panel to reset their password, but you can customize the generated email to send users to the frontend of your app instead by [overriding the email HTML](/authentication/email#forgot-password).
...
```graphql
mutation {
  forgotPassword[collection-singular-label](email: "dev@frogbot.com")
}
```
...
<Note>
  **Note:** if you do not have a `config.serverURL` set, FrogBot will attempt to
  ...
  are calling `frogbot.forgotPassword()` via the Local API. If you do not have a
