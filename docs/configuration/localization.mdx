...
When [Localization](/configuration/localization) is enabled and a collection or global has **drafts enabled** and contains at least one localized field, FrogBot automatically localizes the `_status` field. This means publication status is tracked independently per locale, so you can publish in English while keeping Spanish as a draft.
...
The same parameters are available on `frogbot.updateGlobal()` for globals, as well as on the REST and Local APIs.
