...
description: Add and maintain as many locales as you need by adding Localization to your FrogBot Config, set options for default locale, fallbacks, fields and more.
...
Localization is one of the most important features of a modern CMS. It allows you to manage content in multiple languages, then serve it to your users based on their requested language. This is similar to [I18n](/configuration/i18n), but instead of managing translations for your application's interface, you are managing translations for the data itself.

With Localization, you can begin to serve personalized content to your users based on their specific language preferences, such as a multilingual website or multi-site application. There are no limits to the number of locales you can add to your FrogBot project.

To configure Localization, use the `localization` key in your [FrogBot Config](/configuration/overview):
...
Add the `localization` property to your FrogBot Config to enable Localization project-wide. You'll need to provide a list of all locales that you'd like to support as well as set a few other options.

To configure locales, use the `localization.locales` property in your [FrogBot Config](/configuration/overview):
...
FrogBot Localization works on a **field** level—not a document level. In addition to configuring the base FrogBot Config to support Localization, you need to specify each field that you would like to localize.
...
When [Localization](/configuration/localization) is enabled and a collection has **drafts enabled** and contains at least one localized field, FrogBot automatically localizes the `_status` field. This means publication status is tracked independently per locale, so you can publish in English while keeping Spanish as a draft.
...
If localization is not configured, or the collection has no localized fields, the `status` field remains a single string (`'draft'` or `'published'`) representing document status across all locales.
...
The same parameters are available through the REST and Local APIs.
...
valid locale as provided to your base FrogBot Config, or `'null'`, `'false'`, or `'none'` to disable falling back.
