...
description: FrogBot provides a querying language through all APIs, allowing you to filter or search through documents within a Collection.
keywords: query, documents, overview, documentation, Content Management System, cms, headless, javascript, node, react, nextjs
---

In FrogBot, "querying" means filtering or searching through Documents within a [Collection](/configuration/collections). The querying language in FrogBot is designed to be simple and powerful, allowing you to filter Documents with extreme precision through an intuitive and standardized structure.

FrogBot provides three common APIs for querying your data:
...
Each of these APIs share the same underlying querying language, and fully support all of the same features. This means that you can learn FrogBot's querying language once, and use it across any of the APIs that you might use.
...
<Note>
  You can also use queries within [Access Control](/access-control/overview)
  functions.
</Note>
...
| `within`             | For Point Fields, filters documents based on whether points are inside the given area defined in GeoJSON. [Example](/fields/point#querying-within).     |
| `intersects`         | For Point Fields, filters documents based on whether points intersect with the given area defined in GeoJSON. [Example](/fields/point#querying-intersects). |

<Tip>
  If you know your users will be querying on certain fields a lot, add `index:
  true` to the Field Config. This will speed up searches using that field
  immensely. [More details](/database/indexes).
</Tip>
...
Writing queries in FrogBot is simple and consistent across all APIs, with only minor differences in syntax between them.
...
With the [REST API](/rest-api/overview), you can use the full power of FrogBot queries, but they are written as query strings instead:
...
Set the [Depth](/queries/depth) to only the level that you need to avoid populating unnecessary related documents.
...
To learn more, see the [Depth documentation](/queries/depth).

### Limit

Set the [Limit](/queries/pagination#limit) if you can reliably predict the number of matched documents, such as when querying on a unique field.
...
<Tip>
  Use in combination with `pagination: false` for best performance when querying
  by unique fields.
</Tip>

To learn more, see the [Limit documentation](/queries/pagination#limit).

### Select

Use the [Select API](/queries/select) to only process and return the fields you need.
...
To learn more, see the [Select documentation](/queries/select).

### Pagination

[Disable Pagination](/queries/pagination#disabling-pagination) if you can reliably predict the number of matched documents, such as when querying on a unique field.
...
<Tip>
  Use in combination with `limit: 1` for best performance when querying by
  unique fields.
</Tip>

To learn more, see the [Pagination documentation](/queries/pagination).
