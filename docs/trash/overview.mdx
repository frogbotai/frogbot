...
description: Enable soft deletes for your collections to mark documents as deleted without permanently removing them.
...
When enabled, FrogBot automatically injects a deletedAt field into the collection's schema. This timestamp is set when a document is soft-deleted, and cleared when the document is restored.
...
When deleting a document from the main collection List View, FrogBot will soft-delete the document by default. A checkbox in the delete confirmation modal allows users to skip the trash and permanently delete instead.

## API Support

Soft deletes are fully supported across all FrogBot APIs: **Local**, **REST**, and **GraphQL**.
