...
description: IDE support for FrogbotComponent import paths - inline validation, autocomplete, and go-to-definition.
...
FrogBot ships an optional TypeScript Language Service Plugin (`@frogbotai/typescript-plugin`) that enhances your IDE experience when working with custom components. It understands FrogBot's `FrogbotComponent` import path conventions and provides:
...
```bash
pnpm add -D @frogbotai/typescript-plugin
```
...
{
  "compilerOptions": {
    "plugins": [{ "name": "next" }, { "name": "@frogbotai/typescript-plugin" }]
  }
}
```
...
| Package import          | `'@frogbotai/ui/rsc#MyComponent'` | Resolved via node_modules      |
...
```json
{
  "compilerOptions": {
    "plugins": [
      {
        "name": "@frogbotai/typescript-plugin",
        "baseDir": "./src"
      }
    ]
  }
...
The plugin detects `FrogbotComponent` positions by checking the contextual type of string literals in your config. Any string typed as `FrogbotComponent`, `CustomComponent`, or any type that resolves to the same component-path union shape is automatically validated.

This means the plugin works everywhere FrogBot expects a component path, including collection field components, admin panel components, dashboard widgets, and custom views, without needing to hardcode specific config positions.
