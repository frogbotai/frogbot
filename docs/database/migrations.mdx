...
{
  "scripts": {
    "frogbot": "cross-env PAYLOAD_CONFIG_PATH=src/frogbot.config.ts frogbot"
  }
}
```
...
```ts
import { MigrateUpArgs, MigrateDownArgs } from '@frogbot/your-db-adapter'
...
```ts
import { type MigrateUpArgs } from '@frogbot/db-mongodb'
...
```ts
import { type MigrateUpArgs, sql } from '@frogbot/db-postgres'
...
```ts
import { type MigrateUpArgs, sql } from '@frogbot/db-sqlite'
