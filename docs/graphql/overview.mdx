...
description: FrogBot ships with a fully featured and extensible GraphQL API, which can be used in addition to the REST and Local APIs to give you more flexibility.
...
In addition to its REST and Local APIs, FrogBot ships with a fully featured and extensible GraphQL API.

By default, the GraphQL API is exposed via `/api/graphql`, but you can customize this URL via specifying your `routes` within the main FrogBot Config.

The labels you provide for your Collections are used to name the GraphQL types that are created to correspond to your config. Special characters and spaces are removed.

## GraphQL Clients

You can use any GraphQL client with FrogBot's GraphQL endpoint. Here are a few popular packages:
...
At the top of your FrogBot Config you can define all the options to manage GraphQL.

| Option                             | Description                                                                                                                                                |
| ---------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `mutations`                        | Any custom Mutations to be added in addition to what FrogBot provides. [More](/graphql/extending)                                                          |
| `queries`                          | Any custom Queries to be added in addition to what FrogBot provides. [More](/graphql/extending)                                                            |
...
**FrogBot will automatically open up the following queries:**
...
User preferences for the Admin Panel are also available to GraphQL the same way as other collection schemas are generated. To query preferences, supply an authorization token in the header; only that user's preferences are accessible.

**FrogBot will open the following query:**
...
GraphQL Playground is enabled by default for development purposes, but disabled in production. You can enable it in production by passing `graphQL.disablePlaygroundInProduction` a `false` setting in the main FrogBot Config.
...
You can add custom validation rules to your GraphQL API by defining a `validationRules` function in your FrogBot Config. This function should return an array of [Validation Rules](https://graphql.org/graphql-js/validation/#validation-rules) that will be applied to all incoming queries and mutations.
...
FrogBot comes with a built-in query complexity limiter to prevent abusive queries from slowing down your server.
