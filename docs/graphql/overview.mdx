...
```ts
import type { GlobalConfig } from 'frogbot';
...
FrogBot comes with a built-in query complexity limiter to prevent bad people from trying to slow down your server by running massive queries. To learn more, [click here](/production/preventing-abuse#limiting-graphql-complexity).
