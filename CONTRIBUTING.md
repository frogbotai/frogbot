...
- For code changes, run relevant tests followed by `pnpm prettier:write && pnpm lint:fix`. Agents must delegate lint and required type-checking to the `lint` subagent, including `pnpm lint:fix`. During implementation, typecheck only affected workspaces with `pnpm --filter <workspace-name> typecheck`; do not use the root `pnpm typecheck` as a routine intermediate check. Run the root typecheck at most once, during final verification, when the change requires repository-wide coverage.
